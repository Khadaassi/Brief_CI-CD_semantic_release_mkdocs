...
def test_create_item(client: TestClient) -> None:
    ...
    response = client.post("/items/", json={"nom": "Test Item", "prix": 19.99})
...
def test_create_item_invalid_price(client: TestClient) -> None:
    ...
    response = client.post("/items/", json={"nom": "Test Item", "prix": -10})
    assert response.status_code == 422


def test_create_item_empty_name(client: TestClient) -> None:
    """Teste que la création échoue avec un nom vide."""
    response = client.post("/items/", json={"nom": "", "prix": 10})
    assert response.status_code == 422
...
def test_get_item_by_id(client: TestClient) -> None:
    ...
    create_response = client.post("/items/", json={"nom": "Test Item", "prix": 15.0})
...
def test_update_item(client: TestClient) -> None:
    ...
    create_response = client.post("/items/", json={"nom": "Original", "prix": 10.0})
    item_id = create_response.json()["id"]

    update_response = client.put(
        f"/items/{item_id}", json={"nom": "Updated", "prix": 25.0}
    )
...
def test_update_item_partial(client: TestClient) -> None:
    ...
    update_response = client.put(f"/items/{item_id}", json={"prix": 30.0})
    assert update_response.status_code == 200
    data = update_response.json()
...
def test_update_item_not_found(client: TestClient) -> None:
    ...
    response = client.put("/items/999", json={"nom": "Updated", "prix": 25.0})
    assert response.status_code == 404


def test_delete_item(client: TestClient) -> None:
    """Teste la suppression d'un article existant."""
    create_response = client.post("/items/", json={"nom": "To Delete", "prix": 10.0})
    item_id = create_response.json()["id"]
